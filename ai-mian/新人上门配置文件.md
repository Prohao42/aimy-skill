...
**方式一：对话中输入 `/mode` 命令（推荐）**
```
/mode rookie    切到菜鸟模式（详细解释、教育导向）
/mode veteran   切到老鸟模式（简洁直接、只出高质量洞）
```

**方式二：通过 `settings.json` 持久固定**
```json
"CLAUDE_MODE": "rookie"    // 菜鸟模式（默认）：详细解释，教育导向
"CLAUDE_MODE": "veteran"   // 老鸟模式：简洁直接，跳过基础
```
...
|------|---------|---------|
| 输出 | 详细解释每步原理 | 直达核心技术 |
| 漏洞关注 | 什么洞都分析，重在学习 | **专注高质量洞**，水洞不展开 |
| 代码 | 完整注释+说明 | 只给关键 PoC |
| 适合场景 | 入门、教学、学习新方向 | 挖洞、渗透、赶活 |

### 推荐场景

| 场景 | 推荐模式 |
|------|---------|
| 刚入门安全、接触新漏洞类型 | rookie |
| 指导新人、写教学材料 | rookie |
| 有经验、追求效率 | veteran |
| 日常挖洞/渗透、赶时间、不想看水洞 | veteran |
